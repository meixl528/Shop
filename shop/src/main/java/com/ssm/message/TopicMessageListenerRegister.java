...
public class TopicMessageListenerRegister extends HashMap<MessageListener, Collection<? extends Topic>>
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        //TopicMonitor tt= TestMonitorServiceImpl.class.getAnnotation(TopicMonitor.class);
        monitors.forEach((k, v) -> {
            Class<?> clazz = v.getClass();
            //为何找不到 注解?
            //TopicMonitor tm = clazz.getAnnotation(TopicMonitor.class);
            TopicMonitor tm = applicationContext.findAnnotationOnBean(k, TopicMonitor.class);
            System.out.println("TopicMonitor = "+tm);
            ...
            //listeners.add((ITopicMessageListener) v);
        });
        
        //Map<String, ITopicMessageListener> map = applicationContext.getBeansOfType(ITopicMessageListener.class);
        if (listeners != null) {
